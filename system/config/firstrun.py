...
def download_file(url: str, dest_path: str, timeout: int = 30, retries: int = 3, progress_callback=None) -> bool:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"HTTP error during download: {url}. Details: {e!s}")
        except ValueError as e:
            print(f"File validation error: {e!s}")
            if Path(dest_path).exists():
                Path(dest_path).unlink()  # Remove incomplete file
            break  # Stop retrying on file validation errors
        except Exception as e:
            print(f"Unexpected error during download: {e!s}")
...
def update_tts_engines(engine):
    ...
    try:
        ...
        print(f"[{branding}TTS] Error updating TTS engine configuration: {e!s}")
...
def set_firstrun_model_false():
    ...
    try:
        ...
        print(f"[{branding}TTS] Error saving firstrun configuration: {e!s}")
...
with open(AlltalkConfig.default_config_path(), encoding="utf-8") as file:
