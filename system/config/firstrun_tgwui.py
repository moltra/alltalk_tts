...
def download_file(url: str, dest_path: str, timeout: int = 30) -> bool:
    ...
    try:
        ...
        progress_bar = tqdm(total=total_size, unit="iB", unit_scale=True, desc=f"Downloading {Path(dest_path).name}")
        ...
        if progress_bar.n != total_size and total_size > 0:
            raise requests.exceptions.RequestException(f"Incomplete download: {progress_bar.n}/{total_size} bytes")
        return True
    ...
    try:
        if model_name == "piper":
            ...
            if not download_file(
                "https://coqui.gateway.scarf.sh/v0.6.1_models/tts_models--en--vctk--vits.zip", zip_path
            ):
                return False
            ...
            try:
                ...
                print(f"Error extracting VITS model: {e!s}")
                return False
...
def update_tts_engines(engine: str) -> bool:
    ...
    try:
        ...
        print(f"Error updating TTS engine configuration: {e!s}")
        return False
    ...
    try:
        ...
        with open(config_path) as f:
            ...
            if saved_config.get("firstrun_model", True):
                print("Warning: Failed to save firstrun_model=False")
        ...
        print(f"Error saving firstrun configuration: {e!s}")
        return False
...
def warning_message(branding: str):
    ...
    print(
        f"[{branding}TTS] \033[93mOn Linux ignore the \033[0m'sparse_attn requires a torch version' \033[93mand\033[0m"
    )
...
def main():
    ...
    try:
        # Load configuration
        with open(AlltalkConfig.default_config_path()) as file:
