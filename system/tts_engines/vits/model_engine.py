...
try:
    ...
    print(
        "[Startup] \033[91mWarning\033[0m Could not find the TTS module. Make sure to install the requirements for the alltalk_tts extension.",
        "[Startup] \033[91mWarning\033[0m Linux / Mac:\npip install -r extensions/alltalk_tts/requirements.txt\n",
        "[Startup] \033[91mWarning\033[0m Windows:\npip install -r extensions\\alltalk_tts\\requirements.txt\n",
        "[Startup] \033[91mWarning\033[0m If you used the one-click installer, paste the command above in the terminal window launched after running the cmd_ script. On Windows, that's cmd_windows.bat.",
    )
    raise
...
class tts_class:
    def __init__(self):
        ...
        ############################################################################
        with open(
            os.path.join(self.this_dir, "..", "..", "..", "config", "engines", "vits", "model_settings.json")
        ) as f:
    ...
    async def setup(self):
        ...
        if self.selected_model:
            ...
            # ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
                ...
                print(
                    f"[{self.branding}ENG] \033[91mError\033[0m: Selected model '{self.selected_model}' not found in the models folder."
                )
    ...
    def scan_models_folder(self):
        self.debug_func_entry()
        models_folder = (
            self.main_dir / "models" / "vits"
        )  # Edit to match the name of your folder where voices are stored
        ...
        for subfolder in models_folder.iterdir():
            if subfolder.is_dir():
                ...
                if not (subfolder / "config.json").exists():
                    print(
                        f"[{self.branding}ENG] \033[91mWarning\033[0m: Model folder '{model_name}' missing config.json"
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"[{self.branding}ENG] \033[91mWarning\033[0m: Couldn't read config.json for '{model_name}': {e!s}"
                    )
                    continue
                ...
                if not (has_model_file or has_model_file_tar):
                    print(
                        f"[{self.branding}ENG] \033[91mWarning\033[0m: Model folder '{model_name}' missing model file"
                    )
                    continue
        ...
        try:
            ...
            if model_type in ["vits_multi_speaker", "vits_multi_language"]:
                ...
                if speaker_ids_path.exists():
                    with open(speaker_ids_path) as f:
                        speaker_ids = json.load(f)
                    voices = [speaker for speaker in speaker_ids.keys() if speaker != "ED\n"]
                    print(
                        f"[{self.branding}ENG] Found {len(voices)} voices in speaker_ids.json"
                    ...
                    print(
                        f"[{self.branding}ENG] Created {len(voices)} default voice names from config"
                    ) if self.debug_tts else None
                else:
                    print(
                        f"[{self.branding}ENG] No speaker information found for multi-speaker model '{model_name}'"
                    ) if self.debug_tts else None
            ...
            print(f"[{self.branding}ENG] Exception: {e!s}")
            return ["No Voices Found"]
        ...
        def get_model_path(model_dir, model_type):
            ...
            if model_type == "vits_multi_language":
                ...
                if (model_dir / "model_file.pth.tar").exists():
                    print("Found model_file.pth.tar") if self.debug_tts else None
                    return model_dir / "model_file.pth.tar"

            # Check for regular .pth files
            if (model_dir / "model_file.pth").exists():
                print("Found model_file.pth") if self.debug_tts else None
                return model_dir / "model_file.pth"
            elif (model_dir / "model.pth").exists():
                print("Found model.pth") if self.debug_tts else None
    ...
    async def generate_tts(
        ...
        if not self.is_tts_model_loaded:  # Check if a model is loaded and error out if not.
            ...
            print(error_message)
            raise HTTPException(
                status_code=400, detail="You currently have no TTS model loaded."
            )  # Raise an exception with a meaningful HTTP status code
        ...
        if self.current_model_loaded.startswith("vits"):
            ...
            if model_type == "vits_multi_speaker":
                ...
                if speaker_ids_path.exists():
                    ...
                    print(
                        f"[{self.branding}ENG] \033[91mWarning\033[0m: speaker_ids.json not found for model '{model_name}'."
                    )
                    available_speakers = []
                ...
                if voice in available_speakers:
                    ...
                    if available_speakers:
                        ...
                        print(
                            f"[{self.branding}ENG] Incorrect or no voice specified, using default voice: {speaker_idx}"
                        )
                    else:
                        speaker_idx = None
        ...
        if self.lowvram_enabled and self.device == "cuda" and self.tts_narrator_generatingtts == False:
            await self.handle_lowvram_change()
        self.tts_generating_lock = (
            False  # Unlock the TTS generation queue to allow TTS generation requests to come in again.
        )
