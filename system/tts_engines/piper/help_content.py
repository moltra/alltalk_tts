...
class AllTalkHelpContent:
    ...
    HELP_PAGE = """
    ## 🎯 Piper TTS Engine Help

    Piper is a fast, local neural Text-to-Speech system designed for efficient performance with high-quality output.
    """
    ...
    - **Model Storage**
        - Models are stored in `/alltalk_tts/models/piper/`
        - Each model consists of two required files:
            - `.onnx` file (the voice model)
            - `.onnx.json` file (model configuration)

    - **Output Files**
        - Generated audio is saved to `/alltalk_tts/outputs/`
        - Automatic cleanup available via `Del WAV's older than` setting
        - When enabled, old files are removed on startup
        - Set to `Disabled` to preserve all output files

    ## 🗣️ Voice System

    - **Built-in Voices**
        - Voices are integrated into the models themselves
        - No separate voice files needed
        - Each model represents a complete voice with its own characteristics
        - Models are organized by language and quality levels (x_low, low, medium, high)

    - **Language Support**
        - Wide range of languages available
        - Models named using language codes (e.g., en_US, de_DE)
        - Multiple voice options per language
        - Different quality levels for varying needs
    """
    ...
    - **Quality Levels**
        - `x_low`: Fastest, lowest resource usage
        - `low`: Good balance of speed and quality
        - `medium`: Higher quality, moderate resources
        - `high`: Best quality, higher resource usage
    ...
    - **Model Names**
        ...
        - Indicates language, speaker, and quality level

    - **Best Practices**
        - Choose quality level based on your system capabilities
        - Use appropriate language model for your text

    ## 🔄 Model Downloads

    - Models can be downloaded individually or as language packs
    - All models are sourced from the official Piper repository
    - Downloads include both required files automatically
    - Models can be managed through the Models/Voices Download tab
    """
