...
def group_models_by_language():
    ...
    for model in available_models["models"]:
        ...
        if lang_code not in language_groups:
            language_groups[lang_code] = {"name": get_language_name(lang_code), "models": []}
...
def download_model(model_name, force_download=False):
    ...
    all_files_exists = all(
        os.path.exists(os.path.join(folder_path, os.path.basename(url.split("?")[0]))) for url in files_to_download
    )
    ...
    for idx, model in enumerate(models, 1):
        ...
        for url in model["files_to_download"]:
            try:
                ...
                errors.append(f"Error downloading {file_name}: {e!s}")
...
def piper_model_update_settings(
        ...
        with gr.Tab("Models/Voices Download"):
            with gr.Tab("Language Pack Download"):
                ...
                lang_pack_download_button.click(
                    download_selected_language_pack, inputs=lang_pack_dropdown, outputs=lang_pack_status
                )
            ...
            def show_confirm_cancel(model_name):
                all_files_exists = all(
                    os.path.exists(os.path.join(main_dir, "models", "piper", os.path.basename(file.split("?")[0])))
                    for model in available_models["models"]
