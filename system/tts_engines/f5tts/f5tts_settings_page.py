...
def f5tts_model_update_settings(
    ...
    print(f"[DEBUG] F5TTS Settings saved - File: {settings_file}")
    print(f"[DEBUG] F5TTS Settings saved - Data: {json.dumps(model_config_data, indent=2)[:500]}...")
    ...
    with gr.Blocks(title="f5tts TTS", analytics_enabled=False) as app:
        ...
        with gr.Tab("Models/Voices Download"):
            ...
            def download_model(model_name, force_download=False):
                ...
                model_folder_path = os.path.join(
                    base_folder_path, selected_model["folder_path"]
                ...
                all_files_exists = all(
                    os.path.exists(os.path.join(model_folder_path, file_name)) for file_name in files_to_download
                )
        ...
        with gr.Tab("Reference Text/Sample Manager"):
            with gr.Row():
                ...
                file_list = gr.Dropdown(label="Voice Files", choices=get_voice_files(), value=None)
                refresh_btn = gr.Button("Refresh List")
                model_selector = gr.Dropdown(
                    label="Whisper Model", choices=["base", "medium", "large-v3"], value="base"
                )
            ...
            def save_reference_text(wav_file, text):
                ...
                try:
                    ...
                    return f"Error saving reference text: {e!s}"
            ...
            def delete_reference_text(wav_file):
                ...
                try:
                    ...
                    return f"Error deleting reference text: {e!s}"
            ...
            save_btn.click(save_reference_text, inputs=[file_list, current_text], outputs=status_text).then(
                refresh_file_list, outputs=file_list
            )
            delete_btn.click(delete_reference_text, inputs=[file_list], outputs=status_text).then(
                refresh_file_list, outputs=file_list
            )
            ...
            ).then(fn=refresh_file_list, outputs=file_list)
