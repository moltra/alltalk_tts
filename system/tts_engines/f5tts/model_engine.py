...
def install_and_restart():
    try:
        ...
        subprocess.check_call([sys.executable, "-m", "pip", "install", "vocos"])

        # Then install F5-TTS
        print("Installing F5-TTS...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "git+https://github.com/SWivid/F5-TTS.git"])
        ...
        print("########################################################")
        print(f"Failed to install required packages: {e!s}")
        print("########################################################")
...
class tts_class:
    def __init__(self):
        ...
        self.this_dir = Path(
            __file__
        ).parent.resolve()  # Sets up self.this_dir as a variable for the folder THIS script is running in.
        self.main_dir = (
            Path(__file__).parent.parent.parent.parent.resolve()
        )  # Sets up self.main_dir as a variable for the folder AllTalk is running in
        self.device = (
            "cuda" if torch.cuda.is_available() else "cpu"
        )  # Sets up self.device to cuda if torch exists with Nvidia/CUDA, otherwise sets to cpu
        self.cuda_is_available = (
            torch.cuda.is_available()
        )  # Sets up cuda_is_available as a True/False to track if Nvidia/CUDA was found on the system
        self.tts_generating_lock = (
            False  # Used to lock and unlock the tts generation process at the start/end of tts generation.
        )
        ...
        ]  # A list of ALL the TTS engines available to be loaded by AllTalk
        self.engine_loaded = tts_engines_data[
            "engine_loaded"
        ]  # In "tts_engines.json" what is the currently set TTS engine loading into AllTalk
        self.selected_model = tts_engines_data[
            "selected_model"
        ]  # In "tts_engines.json" what is the currently set TTS model loading into AllTalk
        ############################################################################
        # DONT CHANGE # Pull out all the settings for the currently set TTS engine #
        ############################################################################
        with open(
            os.path.join(self.this_dir, "..", "..", "..", "config", "engines", "f5tts", "model_settings.json")
        ) as f:
            tts_model_loaded = json.load(f)
        # Access the model details
        self.manufacturer_name = tts_model_loaded["model_details"][
            "manufacturer_name"
        ]  # The company/person/body that generated the TTS engine/models etc
        self.manufacturer_website = tts_model_loaded["model_details"][
            "manufacturer_website"
        ]  # The website of the company/person/body where people can find more information
        # Access the features the model is capable of:
        self.audio_format = tts_model_loaded[
            "model_capabilties"
        ][
            "audio_format"
        ]  # This details the audio format your TTS engine is set to generate TTS in e.g. wav, mp3, flac, opus, acc, pcm. Please use only 1x format.
        self.deepspeed_capable = tts_model_loaded["model_capabilties"][
            "deepspeed_capable"
        ]  # Is your model capable of DeepSpeed
        self.deepspeed_available = (
            "deepspeed" in globals()
        )  # When we did the import earlier, at the top of this script, was DeepSpeed available for use
        self.generationspeed_capable = tts_model_loaded["model_capabilties"][
            "generationspeed_capable"
        ]  # Does this TTS engine support changing the speed of the generated TTS
        self.languages_capable = tts_model_loaded[
            "model_capabilties"
        ][
            "languages_capable"
        ]  # Are the actual models themselves capable of generating in multiple languages OR is each model language specific
        self.lowvram_capable = tts_model_loaded["model_capabilties"][
            "lowvram_capable"
        ]  # Is this engine capable of using low VRAM (moving the model between CPU And GPU memory)
        self.multimodel_capable = tts_model_loaded["model_capabilties"][
            "multimodel_capable"
        ]  # Is there just the one model or are there multiple models this engine supports.
        self.repetitionpenalty_capable = tts_model_loaded["model_capabilties"][
            "repetitionpenalty_capable"
        ]  # Is this TTS engine capable of changing the repititon penalty
        self.streaming_capable = tts_model_loaded["model_capabilties"][
            "streaming_capable"
        ]  # Is this TTS engine capabale of generating streaming audio
        self.temperature_capable = tts_model_loaded["model_capabilties"][
            "temperature_capable"
        ]  # Is this TTS engine capable of changing the temperature of the models
        self.multivoice_capable = tts_model_loaded["model_capabilties"][
            "multivoice_capable"
        ]  # Are the models multi-voice or single vocice models
        self.pitch_capable = tts_model_loaded["model_capabilties"][
            "pitch_capable"
        ]  # Is this TTS engine capable of changing the pitch of the genrated TTS
        # Access the current enginesettings
        self.def_character_voice = tts_model_loaded["settings"][
            "def_character_voice"
        ]  # What is the current default main/character voice that will be used if no voice specified.
        self.def_narrator_voice = tts_model_loaded["settings"][
            "def_narrator_voice"
        ]  # What is the current default narrator voice that will be used if no voice specified.
        self.deepspeed_enabled = tts_model_loaded["settings"][
            "deepspeed_enabled"
        ]  # If its available, is DeepSpeed enabled for the TTS engine
        self.engine_installed = tts_model_loaded["settings"][
            "engine_installed"
        ]  # Has the TTS engine been setup/installed (not curently used)
        self.generationspeed_set = tts_model_loaded["settings"][
            "generationspeed_set"
        ]  # What is the set/stored speed for generation.
        self.lowvram_enabled = tts_model_loaded["settings"][
            "lowvram_enabled"
        ]  # If its available, is LowVRAM enabled for the TTS engine
        # Check if someone has enabled lowvram on a system that's not CUDA enabled
        self.lowvram_enabled = (
            False if not torch.cuda.is_available() else self.lowvram_enabled
        )  # If LowVRAM is mistakenly set and CUDA is not available, this will force it back off
        self.repetitionpenalty_set = tts_model_loaded["settings"][
            "repetitionpenalty_set"
        ]  # What is the currenly set repitition policy of the model (If it support repetition)
        self.temperature_set = tts_model_loaded["settings"][
            "temperature_set"
        ]  # What is the currenly set temperature of the model (If it support temp)
        self.pitch_set = tts_model_loaded["settings"][
            "pitch_set"
        ]  # What is the currenly set pitch of the model (If it support temp)
        # Gather the OpenAI API Voice Mappings
        self.openai_alloy = tts_model_loaded["openai_voices"][
            "alloy"
        ]  # The TTS engine voice that will be mapped to Open AI Alloy voice
        self.openai_echo = tts_model_loaded["openai_voices"][
            "echo"
        ]  # The TTS engine voice that will be mapped to Open AI Echo voice
        self.openai_fable = tts_model_loaded["openai_voices"][
            "fable"
        ]  # The TTS engine voice that will be mapped to Open AI Fable voice
        self.openai_nova = tts_model_loaded["openai_voices"][
            "nova"
        ]  # The TTS engine voice that will be mapped to Open AI Nova voice
        self.openai_onyx = tts_model_loaded["openai_voices"][
            "onyx"
        ]  # The TTS engine voice that will be mapped to Open AI Onyx voice
        self.openai_shimmer = tts_model_loaded["openai_voices"][
            "shimmer"
        ]  # The TTS engine voice that will be mapped to Open AI Shimmer voice
        ###################################################################
        ...
        self.branding = configfile_data.get(
            "branding", ""
        )  # Sets up self.branding for outputting the name stored in the "confgnew.json" file, as used in print statements.
        self.params = configfile_data  # Loads in the curent "confgnew.json" file to self.params.
        self.debug_tts = configfile_data.get("debugging").get(
            "debug_tts"
        )  # Can be used within this script as a True/False flag for generally debugging the TTS generation process.
        self.debug_tts_variables = configfile_data.get("debugging").get(
            "debug_tts_variables"
        )  # Can be used within this script as a True/False flag for generally debugging variables (if you wish).
        ...
        self.f5_model_cfg = dict(dim=1024, depth=22, heads=16, ff_mult=2, text_dim=512, conv_layers=4)
        # Add E2-TTS model configuration
        self.e2_model_cfg = dict(dim=1024, depth=24, heads=16, ff_mult=4)
    ...
    async def setup(self):
        ...
        if self.selected_model and self.selected_model != "No Models Found":
            try:
                ...
                print(f"[{self.branding}ENG] \033[91mError loading model: {e!s}\033[0m")
        ...
        # ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        self.setup_has_run = (
            True  # Flag that setup has run, so the /api/ready endpoint will send a "Ready" status and load the webui
        )
    ...
    async def handle_lowvram_change(self):
        ...
        try:
            if self.lowvram_enabled and self.device == "cuda":
                ...
                if hasattr(self, "vocoder"):
                    ...
                    try:
                        ...
                        print(f"[{self.branding}Debug] Vocoder move warning: {e!s}") if self.debug_tts else None
            ...
            print(f"[{self.branding}ENG] \033[91mWarning during model movement: {e!s}\033[0m")
    ...
    def scan_models_folder(self):
        ...
        if not models_dir.exists():
            ...
            print(
                f"[{self.branding}ENG] \033[91mWarning\033[0m: Please use the Gradio inteface to download/select a model."
            )
        ...
        for model_dir in models_dir.glob("*tts_v*"):
            if model_dir.is_dir():
                ...
                if model_files and all(f.exists() for f in [vocab_file, vocos_config, vocos_model]):
                    ...
                    print(
                        f"[{self.branding}ENG] \033[91mWarning\033[0m: Model folder '{model_dir.name}' is missing required files"
                    )
        ...
        try:
            ...
            for f in directory.glob("*.wav"):
                if has_reference_text(f):
                    ...
                    print(
                        f"[{self.branding}ENG] Warning: {f.name} does not have a matching reference text file"
            ...
            for folder in directory.iterdir():
                if folder.is_dir():
                    ...
                    if valid_pairs:
                        ...
                        print(
                            f"[{self.branding}ENG] Warning: Folder {folder.name} has no valid wav+reference.txt pairs"
                        )
    ...
    async def api_manual_load_model(self, model_name):
        ...
        print(
            f"[{self.branding}ENG]\033[94m Model/Engine :\033[93m {model_name}\033[94m loading into\033[93m",
            self.device,
            "\033[0m",
        )
        ...
        if not model_dir.exists():
            ...
            print(
                f"[{self.branding}ENG] \033[93mPlease download an F5-TTS model file in the Gradio interface section for the F5-TTS engine.\033[0m"
            )
        ...
        try:
            ...
            if not model_files:
                print(
                    f"[{self.branding}ENG] \033[91mError\033[0m: No model's safetensors file was found in the F5-TTS models directory."
                )
                print(
                    f"[{self.branding}ENG] \033[93mPlease download an F5-TTS model file in the Gradio interface section for the F5-TTS engine.\033[0m"
                )
            ...
            if is_e2_model:
                ...
                self.model = CFM(
                    transformer=UNetT(**self.e2_model_cfg, text_num_embeds=vocab_size, mel_dim=self.n_mel_channels),
                ...
                self.model = CFM(
                    transformer=DiT(**self.f5_model_cfg, text_num_embeds=vocab_size, mel_dim=self.n_mel_channels),
                    mel_spec_kwargs=mel_spec_kwargs,
                    odeint_kwargs=dict(
                        method=self.ode_method,
                    ),
            ...
            backward_compat_keys = ["mel_spec.mel_stft.mel_scale.fb", "mel_spec.mel_stft.spectrogram.window"]
            ...
            print(f"[{self.branding}ENG] \033[91mError loading model: {e!s}\033[0m")
            raise HTTPException(status_code=500, detail=f"Error loading model: {e!s}")
    ...
    async def handle_tts_method_change(self, tts_method):
        ...
        print(
            f"[{self.branding}ENG] \033[94mLoad time :\033[93m {generate_elapsed_time:.2f} seconds.\033[0m"
            ...
            if clip_short:
                ...
                for non_silent_seg in non_silent_segs:
                    if len(non_silent_wave) > 6000 and len(non_silent_wave + non_silent_seg) > 12000:
                        print(
                            f"[{self.branding}ENG] Audio is over 12s, clipping short. (1)"
                        ) if self.debug_tts else None
                        break
                ...
                if len(non_silent_wave) > 12000:
                    ...
                    for non_silent_seg in non_silent_segs:
                        if len(non_silent_wave) > 6000 and len(non_silent_wave + non_silent_seg) > 12000:
                            print(
                                f"[{self.branding}ENG] Audio is over 12s, clipping short. (2)"
                            ) if self.debug_tts else None
                            break
                        non_silent_wave += non_silent_seg
    ...
    def remove_silence_edges(self, audio, silence_threshold=-42):
        try:
            ...
            print(f"[{self.branding}ENG] Error in remove_silence_edges: {e!s}") if self.debug_tts else None
    ...
    async def infer_process(
        ...
        try:
            ...
            print(
                f"[{self.branding}ENG] Generating audio in {len(gen_text_batches)} batches..."
            ) if self.debug_tts else None
            ...
            print(f"[{self.branding}ENG] Error in infer_process: {e!s}") if self.debug_tts else None
            # Return some default values to avoid unpacking errors
            return np.zeros(1000), self.target_sample_rate, None
    ...
    async def infer_batch_process(
        ...
        for gen_text in gen_text_batches:
            ...
            if len(gen_text.encode("utf-8")) < 10:
                local_speed = 0.3
                print(
                    f"[{self.branding}Debug] Short text detected, using speed={local_speed}"
                ) if self.debug_tts else None
            else:
            ...
            if fix_duration is not None:
                ...
                print(
                    f"[{self.branding}Debug] ref_text_len={ref_text_len}, gen_text_len={gen_text_len}"
                ) if self.debug_tts else None
                ...
                print(
                    f"[{self.branding}Debug] Estimated duration={estimated_duration} frames"
                ) if self.debug_tts else None
    ...
    async def generate_tts(
        ...
        if language not in ["en", "eng", "zh", "zho", "chi"]:
            print(
                f"[{self.branding}ENG] \033[91mError\033[0m: F5-TTS only supports English (EN) or Chinese (ZH) language TTS generation."
            )
            print(f"[{self.branding}ENG] \033[91mError\033[0m: You may get an error message or gibberish output.")
            print(
                f"[{self.branding}ENG] \033[91mError\033[0m: Please use only EN or ZH/ZHO for the language with F5-TTS."
            )
        ...
        try:
            ...
            ref_audio, processed_ref_text = await self.preprocess_ref_audio_text(str(ref_audio_path), ref_text)
            ...
            generate_elapsed_time = generate_end_time - generate_start_time
            print(
                f"[{self.branding}GEN] \033[94mTTS Generate: \033[93m{generate_elapsed_time:.2f} seconds. \033[94mLowVRAM: \033[33m{self.lowvram_enabled} \033[94mDeepSpeed: \033[33m{self.deepspeed_enabled}\033[0m"
            )
            ...
            print(f"[{self.branding}ENG] \033[91mError: {e!s}\033[0m")
            raise HTTPException(status_code=404, detail=str(e))
        except Exception as e:
            print(f"[{self.branding}ENG] \033[91mError during TTS generation: {e!s}\033[0m")
            raise HTTPException(status_code=500, detail=f"Error during TTS generation: {e!s}")
        finally:
            try:
                # Handle low VRAM mode - move back to CPU if needed
                if self.lowvram_enabled and not self.tts_narrator_generatingtts:
                    print(
                        f"[{self.branding}Debug] Moving models back to CPU after generation"
                    ) if self.debug_tts else None
                    await self.handle_lowvram_change()
                ...
                if self.debug_tts:
                    ...
                    if hasattr(self, "vocoder"):
                        try:
                            ...
                            print(
                                f"[{self.branding}Debug] Final vocoder device: {vocoder_device}"
                            ) if self.debug_tts else None
                        except Exception:
                            # Skip vocoder device checking if it fails
                            pass
            except Exception as e:
                print(f"[{self.branding}Debug] Device verification warning: {e!s}") if self.debug_tts else None
