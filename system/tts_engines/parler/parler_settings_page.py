...
main_dir = Path(
    __file__
).parent.parent.parent.parent.resolve()  # Sets up self.main_dir as a variable for the folder AllTalk is running in
...
def parler_model_update_settings(
    def_character_voice_gr,
    def_narrator_voice_gr,
    lowvram_enabled_gr,
    deepspeed_enabled_gr,
    temperature_set_gr,
    repetitionpenalty_set_gr,
    pitch_set_gr,
    generationspeed_set_gr,
    alloy_gr,
    echo_gr,
    fable_gr,
    nova_gr,
    ...
    print(f"[DEBUG] Parler Settings saved - File: {settings_file}")
    print(f"[DEBUG] Parler Settings saved - Data: {json.dumps(model_config_data, indent=2)[:500]}...")
    ...
    with gr.Blocks(title="parler TTS", analytics_enabled=False) as app:
        with gr.Tab("Default Settings"):
            with gr.Row():
                ...
                deepspeed_enabled_gr = gr.Radio(
                    choices={"Enabled": "true", "Disabled": "false"},
                    label="DeepSpeed Activate"
                    if model_config_data["model_capabilties"]["deepspeed_capable"]
                    else "DeepSpeed N/A",
                ...
                temperature_set_gr = gr.Slider(
                    ...
                    step=0.05,
                    label="Temperature"
                    if model_config_data["model_capabilties"]["temperature_capable"]
                ...
                repetitionpenalty_set_gr = gr.Slider(
                    ...
                    if model_config_data["model_capabilties"]["repetitionpenalty_capable"]
                    else "Repetition N/A",
                    ...
                    with gr.Group():
                        with gr.Row():
                            details_text = gr.Textbox(
                                ...
                                lines=5,
                                interactive=False,
                                value="Configure default settings and voice mappings for the selected TTS engine. Unavailable options are grayed out based on engine capabilities. See the Help section below for detailed information about each setting.",
                            )
            with gr.Row():
                submit_button = gr.Button("Update Settings")
                output_message = gr.Textbox(label="Output Message", interactive=False, show_label=False)
            submit_button.click(
                parler_model_update_settings,
                inputs=[
                    def_character_voice_gr,
                    def_narrator_voice_gr,
                    lowvram_enabled_gr,
            ...
            with gr.Row():
                with gr.Group():
                    gr.Textbox(
                        ...
                        value=model_config_data["model_details"]["manufacturer_name"],
                        interactive=False,
                    )
                    gr.Textbox(
                        label="Manufacturer Website/TTS Engine Support",
                        value=model_config_data["model_details"]["manufacturer_website"],
                        interactive=False,
                    )
                    gr.Textbox(
                        label="Engine/Model Description",
                        value=model_config_data["model_details"]["model_description"],
                        interactive=False,
                        lines=13,
                    )
                with gr.Column():
                    with gr.Row():
                        gr.Textbox(
                            label="DeepSpeed Capable",
                            value="Yes" if features_list["deepspeed_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Pitch Capable",
                            value="Yes" if features_list["pitch_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Generation Speed Capable",
                            value="Yes" if features_list["generationspeed_capable"] else "No",
                            interactive=False,
                        )
                    with gr.Row():
                        gr.Textbox(
                            label="Repetition Penalty Capable",
                            value="Yes" if features_list["repetitionpenalty_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Multi Languages Capable",
                            value="Yes" if features_list["languages_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Streaming Capable",
                            value="Yes" if features_list["streaming_capable"] else "No",
                            interactive=False,
                        )
                    with gr.Row():
                        gr.Textbox(
                            label="Low VRAM Capable",
                            value="Yes" if features_list["lowvram_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Temperature Capable",
                            value="Yes" if features_list["temperature_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Multi Model Capable Engine",
                            value="Yes" if features_list["multimodel_capable"] else "No",
                            interactive=False,
                        )
                    with gr.Row():
                        gr.Textbox(
                            label="Multi Voice Capable Models",
                            value="Yes" if features_list["multivoice_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Default Audio output format",
                            value=model_config_data["model_capabilties"]["audio_format"],
                            interactive=False,
                        )
                    ...
                    with gr.Row():
                        gr.Textbox(
                            ...
                            value="Yes" if features_list["windows_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Linux Support",
                            value="Yes" if features_list["linux_capable"] else "No",
                            interactive=False,
                        )
                        gr.Textbox(
                            label="Mac Support",
                            value="Yes" if features_list["mac_capable"] else "No",
                            interactive=False,
                        )
            with gr.Row():
        ...
        with gr.Tab("Models/Voices Download"):
            ...
            def download_model(model_name, force_download=False):
                ...
                selected_model = next(
                    model for model in available_models["models"] if model["model_name"] == model_name
                )
                ...
                all_files_exists = all(
                    os.path.exists(os.path.join(folder_path, os.path.basename(url.split("?")[0])))
            ...
            def show_confirm_cancel(model_name):
                all_files_exists = all(
                    os.path.exists(os.path.join(main_dir, "models", "parler", os.path.basename(file.split("?")[0])))
                    for model in available_models["models"]
                    if model["model_name"] == model_name
                    for file in model["files_to_download"]
                )
                if all_files_exists:
            ...
            download_button.click(
                show_confirm_cancel, inputs=model_dropdown, outputs=[confirm_button, download_button, cancel_button]
            )
            confirm_button.click(
                confirm_download,
                inputs=model_dropdown,
                outputs=[confirm_button, download_button, cancel_button, download_status],
            )
            cancel_button.click(
                cancel_download, inputs=None, outputs=[confirm_button, download_button, cancel_button, download_status]
            )
        ...
        def voices_to_list(voices):
            return [
                f"{i+1}. {voice['voice_name']} - {voice['description']}" for i, voice in enumerate(voices["voices"])
            ]
        ...
        def add_update_voice(voice_name, description, selected_index, button_label):
            ...
            return (
                gr.Dropdown(
                    choices=voices_to_list(voices), value=voices_to_list(voices)[0] if voices["voices"] else None
                ),
                "",
                "",
                -1,
        ...
        with gr.Tab("Voice Editor"):
            ...
            voice_list.change(
                populate_voice_data,
                inputs=voice_list,
                outputs=[voice_name, description, selected_index, voice_name, button_label],
            )
            add_update_button.click(
                add_update_voice,
                inputs=[voice_name, description, selected_index, button_label],
                outputs=[voice_list, voice_name, description, selected_index, voice_name, button_label],
            )
            delete_button.click(
            ...
            clear_button.click(
                clear_text_boxes, outputs=[voice_name, description, selected_index, voice_name, button_label]
            )
