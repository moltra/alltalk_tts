...
class tts_class:
    def __init__(self):
        ...
        ############################################################################
        with open(
            os.path.join(self.this_dir, "..", "..", "..", "config", "engines", "parler", "model_settings.json")
        ) as f:
        ...
        self.dtype_set = (
            "float16"  # Change this line to modify precision. Will faliback to float32 if device doesnt support float16
        )
    ...
    def scan_models_folder(self):
        ...
        common_required_files = ["config.json", "tokenizer_config.json", "special_tokens_map.json"]
            ...
            if subfolder.is_dir():
                ...
                if all(subfolder.joinpath(file).exists() for file in common_required_files):
                    ...
                    if safetensor_files:
                        ...
                        print(
                            f"[{self.branding}ENG] \033[91mWarning\033[0m: Model folder '{model_name}' has no .safetensors files."
                        )
                else:
                    print(
                        f"[{self.branding}ENG] \033[91mWarning\033[0m: Model folder '{model_name}' is missing common required files."
                    )
    ...
    async def api_manual_load_model(self, model_name):
        ...
        if not model_path.exists():
            ...
            print(
                f"[{self.branding}ENG] \033[93mPlease download a Parler model file in the Gradio interface section for the Parler engine.\033[0m"
            )
        ...
        try:
            ...
            print(f"[{self.branding}ENG] \033[91mError loading model:\033[0m {e!s}")
            return None
    ...
    async def generate_tts(
        ...
        if (
            streaming and wavs is not None
        ):  # This is a fake function as this model doesnt stream but Python demands it otherwise it complains about await functions in the main script
