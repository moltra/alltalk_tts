...
try:
    ...
    try:
        ...
        print(f"[AllTalk TTSDiff] Error: {e!s}")
...
parser.add_argument(
    "--threshold", type=int, default=98, help="Similarity threshold for considering a match (default: 98)"
)
...
if not args.wavfilespath:
    parser.error(
        "[AllTalk TTSDiff] Please specify the path to the wavs output folder file using the --wavfilespath argument."
    )
...
def disclaimer_text():
    print("\nDESCRIPTION: ")
    print("  This script is designed to assist in identifying bad TTS generation ID's from the TTS Generator.\n")
    print(
        "  This will download the Whisper Large-v3 model (if not already downloaded), and use it to compare your text to"
    )
    print(
        "  to the generated TTS. If it finds an issue, it will flag the ID number that has the issue. This does NOT 100%"
    )
    print(
        "  ensure that all TTS generated is perfect, sounds good etc, but it should flag up glaring issues and help with"
    )
    print("  the process of identifying bad TTS generations.")
    print("\nNOTES ON USE: ")
    print("  - Ensure AllTalk is running in its own command prompt/terminal.")
    print(
        "  - This script should be running in its own command prompt/terminal window in the same Python Environment that"
    )
    print("    AllTalk is running in.")
    print(
        "  - After generating your text in the TTS Generator, you need to `Export List to JSON` and save the file in the"
    )
    print("    same folder/directory this script is running from.")
    print(
        "  - You can use `--threshold` with a numerical value from 1 to 100 to set accuracy detection. For example it may"
    )
    print(
        "    detect/transcribe `their` as `there`, which of course, both words sound the same. If you want 100% accuracy"
    )
    print(
        "    you will set 100 as the threshold. You will more than likely want a number in the high 90's. Default is set"
    )
    print("    at 98, which gives a little flexibility.")
    print("  - The JSON file must be named \033[93mttsList.json\033[0m")
    print(
        "  - When you have your ID list, go back into the TTS Generator, correct any lines and regenerate them. If you"
    )
    print(
        "    want to re-test everthing again after re-generating, you will need to export your list again and re-run this"
    )
    print("    script again, against your newly exported JSON list.\n")
    return
...
try:
    ...
    print("[AllTalk TTSDiff] ERROR STARTING SCRIPT:")
    print("[AllTalk TTSDiff] An error occurred importing one or more required libraries.")
    print(
        "[AllTalk TTSDiff] Please ensure you have \033[94mactivated your Python environment \033[0mand \033[94minstalled all requirements.\033[0m"
    )
...
def transcribe_and_compare(file_url, original_text, model, item_id, flagged_ids):
    ...
    has_spoken_punctuation = contains_spoken_punctuation(
        transcribed_text_normalized
    ) and not contains_spoken_punctuation(original_text_normalized)
    ...
    if not is_match:
        ...
        if has_spoken_punctuation:
            print("[AllTalk TTSDiff] Note: Potential incorrect spoken punctuation detected.")
...
def main():
    ...
    try:
        ...
        print(
            "[AllTalk TTSDiff] After generating your text in the TTS Generator, you need to `Export List to JSON` and save the file in the"
        )
    ...
    if flagged_ids:
        print("[AllTalk TTSDiff] \033[94mIDs needing review:\033[0m", ", ".join(map(str, flagged_ids)))
        print("[AllTalk TTSDiff]")
        print("[AllTalk TTSDiff] Review ID's and correct any lines by editing & regenerating as necessary.")
    else:
        print("[AllTalk TTSDiff] \033[94mIDs needing review:\033[0m No issues detected.")
    summary_data = {"flagged_ids": flagged_ids}
