...
def install_requirements():
    ...
    required_packages = ["twisted", "cryptography", "service_identity", "requests", "psutil"]
...
def print_message(message, message_type="standard", component="PRX"):
    ...
    if message_type.startswith("debug_"):
        ...
        if message_type == "debug_func" and "Function entry:" in message:
            ...
            logger.debug(f"{prefix}{BLUE}Debug{RESET} {YELLOW}{message_type}{RESET} {message}")

    elif message_type == "debug":
        logger.debug(f"{prefix}{BLUE}Debug{RESET} {message}")

    elif message_type == "warning":
        logger.warning(f"{prefix}{YELLOW}Warning{RESET} {message}")

    elif message_type == "error":
        logger.error(f"{prefix}{RED}Error{RESET} {message}")
...
class ProxyToInternalPort(proxy.ReverseProxyResource):
    ...
    def getChild(self, path, request):
        try:
            ...
            print_message(
                f"{GREEN}Request: {RED}{method} {decoded_path}{GREEN} from {RESET}{ip} {GREEN}Forwarding to:{RESET} {self.host}:{self.port} {RED}{decoded_path}{RESET}",
                message_type="debug_proxy",
            )
            # print_message(f"Forwarding to: {self.host}:{self.port}{decoded_path}", message_type="debug_proxy")
        except Exception as e:
            print_message(f"Error in getChild: {e!s}", message_type="error")
...
def run_proxy(external_ip_a, internal_ip_a, external_ip_b, internal_ip_b, cert_file_a, cert_file_b):
    ...
    print_message(
        f" |- External IP API    : {'[DISABLED]' if external_ip_a == '0' else f'{external_ip_a}:{api_port_external}'} -> {internal_ip_a}:{api_port}",
        message_type="debug_proxy",
    )
    print_message(
        f" |- External IP Gradio : {'[DISABLED]' if external_ip_b == '0' else f'{external_ip_b}:{gradio_port_external}'} -> {internal_ip_b}:{gradio_port}",
        message_type="debug_proxy",
    )
    print_message(
        f" |- Certificate File A : {cert_file_a} {'[No Certificate So Disabled]' if cert_file_a == '0' else ''}",
        message_type="debug_proxy",
    )
    print_message(
        f" |- Certificate File B : {cert_file_b} {'[No Certificate So Disabled]' if cert_file_b == '0' else ''}",
        message_type="debug_proxy",
    )
    ...
    if external_ip_b != "0" and central_config.proxy_settings.gradio_endpoint.enabled == "Enabled":
        if cert_file_b != "0" and check_certificates(cert_file_b):
            sslContextB = ssl.DefaultOpenSSLContextFactory(
                f"certificates/{cert_file_b}_key.pem", f"certificates/{cert_file_b}_cert.pem"
            )
            ...
            print_message(
                f"Gradio SSL Proxy : {GREEN}From{RESET} {external_ip_b}:{gradio_port_external} {GREEN}->{RESET} {internal_ip_b}:{gradio_port} {GREEN}is now active.{RESET}"
            )
        else:
            proxy_gradio = ProxyToInternalPort(internal_ip_b, gradio_port)
            proxy_site_gradio = server.Site(proxy_gradio)
            ...
            print_message(
                f"Gradio HTTP Proxy : {GREEN}From{RESET} {external_ip_b}:{gradio_port_external} {GREEN}->{RESET} {internal_ip_b}:{gradio_port} {GREEN}is now active.{RESET}"
            )
    ...
    if external_ip_a != "0" and central_config.proxy_settings.api_endpoint.enabled == "Enabled":
        if cert_file_a != "0" and check_certificates(cert_file_a):
            sslContextA = ssl.DefaultOpenSSLContextFactory(
                f"certificates/{cert_file_a}_key.pem", f"certificates/{cert_file_a}_cert.pem"
            )
            ...
            print_message(
                f"API SSL Proxy    : {GREEN}From{RESET} {external_ip_a}:{api_port_external} {GREEN}->{RESET} {internal_ip_a}:{api_port} {GREEN}is now active.{RESET}"
            )
        else:
            proxy_api = ProxyToInternalPort(internal_ip_a, api_port)
            proxy_site_api = server.Site(proxy_api)
            ...
            print_message(
                f"API HTTP Proxy    : {GREEN}From{RESET} {external_ip_a}:{api_port_external} {GREEN}->{RESET} {internal_ip_a}:{api_port} {GREEN}is now active.{RESET}"
            )
        proxies_started = True
    else:
    ...
    if not proxies_started:
        print_message(
            f"No proxies were started. {RED}Both{RESET} API and Gradio proxies are {RED}DISABLED{RESET}.",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cert_file_a",
        required=True,
        help="Certificate file base name for External IP A (without '_key' or '_cert', or '0' for HTTP only)",
    )
    parser.add_argument(
        "--cert_file_b",
        required=True,
        help="Certificate file base name for External IP B (without '_key' or '_cert', or '0' for HTTP only)",
    )
