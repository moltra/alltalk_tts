...
class TestSubprocessSecurity:
    ...
    def test_subprocess_no_shell_true_in_run_pip_commands(self):
        ...
        assert 'subprocess.run(command, shell=True' not in content, \
            "Found vulnerable subprocess.run with shell=True"
        
        # Check that the secure pattern IS present
        assert 'subprocess.run([sys.executable, "-m", "pip"] + command.split()' in content, \
    ...
    def test_no_os_system_in_clear_screen(self):
        ...
        assert 'os.system("cls")' not in content, \
            "Found os.system('cls') - should use subprocess instead"
        
        # Check that os.system("clear") is NOT present
        assert 'os.system("clear")' not in content, \
            "Found os.system('clear') - should use subprocess instead"
        
        # Check that subprocess.run is used instead
        assert 'subprocess.run(["cls"]' in content or 'subprocess.run(["clear"]' in content, \
    ...
    def test_subprocess_uses_list_not_string(self):
        ...
        assert '[sys.executable, "-m", "pip"]' in content, \
            "Secure list argument pattern not found"
