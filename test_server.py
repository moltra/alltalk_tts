...
class AllTalkTester:
    def __init__(self):
        ...
        self.volume_test_cases = [{"volume": 0.5, "should_succeed": True}, {"volume": 1.5, "should_succeed": False}]
    ...
    async def menu(self):
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"Menu operation failed: {e!s}")
    ...
    async def individual_test_menu(self):
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"Individual test selection failed: {e!s}")
    ...
    async def _run_ping_test(self):
        ...
        try:
            ...
            if "unreachable" in output.lower() or "timed out" in output.lower():
                self.log_result("Network", "Ping Test", "WARNING", "Host reachable but some packets lost")
            else:
                self.log_result("Network", "Ping Test", "PASS", "Host responding to ping")

        except subprocess.CalledProcessError as e:
            self.log_result("Network", "Ping Test", "WARNING", f"Ping failed but continuing with tests: {e!s}")
        except Exception as e:
            self.log_result("Network", "Ping Test", "ERROR", f"Unexpected error during ping test: {e!s}")
    ...
    async def _test_server_connection(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                while retries < self.config["retry_count"]:
                    try:
                        async with session.get(
                            f"{self.base_url}/api/ready", timeout=aiohttp.ClientTimeout(total=5)
                        ) as response:
                        ...
                        logger.warning(f"Connection attempt {retries + 1} failed: {e!s}")
                ...
                self.log_result(
                    "Network", "Server Connection", "ERROR", f"Server not responding after {retries} attempts"
                )
                return False

            except Exception as e:
                self.log_result(
                    "Network", "Server Connection", "ERROR", f"Unexpected error testing server connection: {e!s}"
                )
                return False
    ...
    async def _test_current_settings(self) -> dict | None:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.base_url}/api/currentsettings", timeout=aiohttp.ClientTimeout(total=self.config["timeout"])
                ) as response:
                    if response.status == 200:
                        ...
                        self.log_result(
                            "Core API", "Current Settings", "ERROR", f"Unexpected status code: {response.status}"
                        )
                        return None

            except Exception as e:
                self.log_result("Core API", "Current Settings", "ERROR", f"Failed to get current settings: {e!s}")
                return None
    ...
    async def _test_voices(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.base_url}/api/voices", timeout=aiohttp.ClientTimeout(total=self.config["timeout"])
                ) as response:
                    if response.status == 200:
                        ...
                        if voices:
                            self.log_result("Core API", "Voices", "PASS", f"Found {len(voices)} voices")
                            # Store voices for later use in TTS tests
                            self.available_voices = voices
                        else:
                            self.log_result("Core API", "Voices", "WARNING", "No voices found")
                    else:
                        self.log_result("Core API", "Voices", "ERROR", f"Unexpected status code: {response.status}")

            except Exception as e:
                self.log_result("Core API", "Voices", "ERROR", f"Failed to get voices: {e!s}")
    ...
    async def _test_rvc_voices(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.base_url}/api/rvcvoices", timeout=aiohttp.ClientTimeout(total=self.config["timeout"])
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        if "Disabled" in rvc_voices and len(rvc_voices) == 1:
                            self.log_result("Core API", "RVC Voices", "PASS", "RVC is disabled (expected behavior)")
                        elif rvc_voices:
                            self.log_result("Core API", "RVC Voices", "PASS", f"Found {len(rvc_voices)} RVC voices")
                            # Store RVC voices for later use
                            self.available_rvc_voices = rvc_voices
                        else:
                            self.log_result("Core API", "RVC Voices", "WARNING", "No RVC voices found")
                    else:
                        self.log_result("Core API", "RVC Voices", "ERROR", f"Unexpected status code: {response.status}")

            except Exception as e:
                self.log_result("Core API", "RVC Voices", "ERROR", f"Failed to get RVC voices: {e!s}")
    ...
    async def _test_root_endpoint(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    self.base_url, timeout=aiohttp.ClientTimeout(total=self.config["timeout"])
                ) as response:
                    if response.status == 200:
                        content = await response.text()
                        if content:
                            self.log_result("Core API", "Root Endpoint", "PASS", "Root endpoint returning content")
                        else:
                            self.log_result(
                                "Core API", "Root Endpoint", "WARNING", "Root endpoint returned empty content"
                            )
                    else:
                        self.log_result(
                            "Core API", "Root Endpoint", "ERROR", f"Unexpected status code: {response.status}"
                        )

            except Exception as e:
                self.log_result("Core API", "Root Endpoint", "ERROR", f"Failed to access root endpoint: {e!s}")
    ...
    async def _select_test_voice(self) -> str | None:
        """Select a random voice from available voices"""
        if not hasattr(self, "available_voices") or not self.available_voices:
            self.log_result("TTS", "Voice Selection", "ERROR", "No voices available for testing")
            return None
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if result.get("status") == "generate-success":
                            self.log_result(
                                "TTS", "Standard Generation", "PASS", f"Output file: {result.get('output_file_path')}"
                            )
                            # Store successful generation for later tests
                            self.last_generated_file = result.get("output_file_path")
                        else:
                            self.log_result("TTS", "Standard Generation", "ERROR", f"Generation failed: {result}")
                    else:
                        self.log_result(
                            "TTS", "Standard Generation", "ERROR", f"Unexpected status code: {response.status}"
                        )

            except Exception as e:
                self.log_result("TTS", "Standard Generation", "ERROR", f"Failed to generate TTS: {e!s}")
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if result.get("status") == "generate-success":
                            self.log_result(
                                "TTS", "Narrator Mode", "PASS", f"Output file: {result.get('output_file_path')}"
                            )
                            # Store successful generation for later tests
                            self.last_narrator_file = result.get("output_file_path")
                        else:
                            self.log_result("TTS", "Narrator Mode", "ERROR", f"Generation failed: {result}")
                    else:
                        self.log_result("TTS", "Narrator Mode", "ERROR", f"Unexpected status code: {response.status}")

            except Exception as e:
                self.log_result("TTS", "Narrator Mode", "ERROR", f"Failed to generate narrator TTS: {e!s}")
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if chunks_received > 0:
                            self.log_result("TTS", "Streaming Generation", "PASS", f"Received {chunks_received} chunks")
                        else:
                            self.log_result(
                                "TTS", "Streaming Generation", "WARNING", "Stream succeeded but no chunks received"
                            )
                    elif response.status == 400:
                        error_data = await response.json()
                        if "streaming not supported" in error_data.get("error", "").lower():
                            self.log_result(
                                "TTS", "Streaming Generation", "PASS", "Correctly reported streaming not supported"
                            )
                        else:
                            self.log_result("TTS", "Streaming Generation", "ERROR", f"Bad request: {error_data}")
                    else:
                        self.log_result(
                            "TTS", "Streaming Generation", "ERROR", f"Unexpected status code: {response.status}"
                        )

            except Exception as e:
                self.log_result("TTS", "Streaming Generation", "ERROR", f"Failed to test streaming: {e!s}")
    ...
    async def _test_openai_single_case(self, text: str, voice: str, format: str):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                self.log_result(
                    "TTS", f"OpenAI Generation ({voice}, {format})", "ERROR", f"Failed to test OpenAI generation: {e!s}"
                )
    ...
    async def _select_two_voices(self) -> tuple[str, str] | None:
        """Select two different voices for narrator testing."""
        if not hasattr(self, "available_voices") or len(self.available_voices) < 2:
            self.log_result("TTS", "Voice Selection", "SKIP", "Need at least 2 voices for narrator testing")
            return None
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if result.get("status") == "generate-success":
                                ...
                                self.log_result(
                                    "TTS", f"Narrator Test {test_name}", "ERROR", f"Generation failed: {result}"
                                )
                except Exception as e:
                    self.log_result("TTS", f"Narrator Test {test_name}", "ERROR", f"Test failed: {e!s}")
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if result.get("status") == "generate-success":
                                ...
                                self.log_result(
                                    "TTS", f"Text Filter {filter_type}", "ERROR", f"Generation failed: {result}"
                                )
                except Exception as e:
                    self.log_result("TTS", f"Text Filter {filter_type}", "ERROR", f"Test failed: {e!s}")
    ...
    async def _test_autoplay(self):
        ...
        try:
                            ...
                            async with session.post(
                                ...
                                if response.status == 200:
                                    ...
                                    if result.get("status") == "generate-success" and expected_success:
                                        self.log_result(
                                            "TTS", f"Autoplay Volume {volume}", "PASS", "Audio should be playing"
                                        )
                                        ...
                                        self.log_result(
                                            "TTS", f"Autoplay Volume {volume}", "ERROR", f"Unexpected result: {result}"
                                        )
                        except Exception as e:
                            self.log_result("TTS", f"Autoplay Volume {volume}", "ERROR", f"Test failed: {e!s}")
            ...
            logger.error(f"Autoplay testing failed: {e!s}")
                ...
                try:
                    async with session.post(
                        ...
                        if response.status == case["expect_error"]:
                            ...
                            if (
                                error_msg
                                and len(error_msg) < 200
                                and not any(x in error_msg.lower() for x in ["traceback", "exception", "stack"])
                            ):
                                self.log_result(
                                    "Error Cases",
                    ...
                    self.log_result("Error Cases", f"Error Test {case['case']}", "ERROR", f"Test failed: {e!s}")
    ...
    async def _test_additional_language(self):
        ...
        if not hasattr(self, "current_settings") or not self.current_settings.get("languages_capable"):
            self.log_result("TTS", "Additional Language", "SKIP", "Multiple languages not supported by current engine")
            return
            ...
            try:
                async with session.post(
                    ...
                    if response.status == 200:
                        result = await response.json()
                        if result.get("status") == "generate-success":
                            self.log_result("TTS", "Additional Language", "PASS", f"Generated {test_language} audio")
                        else:
                            self.log_result("TTS", "Additional Language", "ERROR", f"Generation failed: {result}")
                    else:
                        self.log_result(
                            "TTS", "Additional Language", "ERROR", f"Unexpected status code: {response.status}"
                        )
            except Exception as e:
                self.log_result("TTS", "Additional Language", "ERROR", f"Test failed: {e!s}")
