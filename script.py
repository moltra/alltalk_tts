...
##################################################################
with open(os.path.join(this_dir, "system", "config", "languages.json"), encoding="utf-8") as f:
...
def update_settings_at(
    ...
    try:
        ...
        print_message(f"Configuration structure error: {e!s}", message_type="error")
        return "Error updating settings: Invalid configuration structure"
    except OSError as e:
        print_message(f"File system error while saving configuration: {e!s}", message_type="error")
        return "Error saving settings: File system error"
    except ValueError as e:
        print_message(f"Invalid value provided for configuration: {e!s}", message_type="error")
...
def update_settings_api(
    ...
    try:
        ...
        upd_config.api_def.api_use_legacy_api = api_use_legacy_api == "AllTalk v1 API (Legacy)"
        ...
        upd_config.api_def.api_output_file_timestamp = api_output_file_timestamp == "Timestamp files"
...
def update_settings_tgwui(activate, autoplay, show_text, language, narrator_enabled):
    ...
    try:
        ...
        error_msg = f"Configuration structure error: {e!s}"
        return_message = "Error updating settings: Invalid configuration structure"
    except OSError as e:
        error_msg = f"File system error while saving configuration: {e!s}"
        return_message = "Error saving settings: File system error"
    except ValueError as e:
        error_msg = f"Invalid value provided for configuration: {e!s}"
...
def update_rvc_settings(
    ...
    try:
        ...
        if rvc_enabled:
            ...
            for idx, file in enumerate(file_urls):
                if not os.path.exists(os.path.join(base_dir, file)):
                    ...
                    print(f"[{config.branding}TTS] Downloading {file}...")  # Print statement for terminal
                    download_file(file_urls[file], os.path.join(base_dir, file))
            download_result = "All RVC Base Files are present." if len(file_urls) > 0 else "All files are present."
        ...
        error_msg = f"Error downloading RVC files: {e!s}"
        return_message = "Error downloading RVC files: Network or server error"
    except json.JSONDecodeError as e:
        error_msg = f"Error reading RVC configuration file: {e!s}"
        ...
        error_msg = f"File system error while saving/creating directories: {e!s}"
...
def update_proxy_settings(
    ...
    try:
        ...
        print_message(f"Error updating proxy settings: {e!s}", message_type="error")
        return f"Error updating proxy settings: {e!s}"
...
print_message("\033[94m    _    _ _ \033[1;35m_____     _ _     \033[0m  _____ _____ ____  ")
print_message("\033[94m   / \\  | | |\033[1;35m_   _|_ _| | | __ \033[0m |_   _|_   _/ ___| ")
print_message("\033[94m  / _ \\ | | |\033[1;35m | |/ _` | | |/ / \033[0m   | |   | | \\___ \\ ")
print_message("\033[94m / ___ \\| | |\033[1;35m | | (_| | |   <  \033[0m   | |   | |  ___) |")
print_message("\033[94m/_/   \\_\\_|_|\033[1;35m |_|\\__,_|_|_|\\_\\ \033[0m   |_|   |_| |____/ ")
print_message("")
...
def run_firsttime_script(tts_model=None):
    ...
    try:
        ...
        if _state["running_on_google_colab"]:
            ...
            firstrun_script_path = os.path.join(this_dir, "system", "config", "firstrun_tgwui.py")
        else:
            firstrun_script_path = os.path.join(this_dir, "system", "config", "firstrun.py")
        ...
        error_msg = f"Error running first-time setup script: {e!s}"
        print_message(error_msg, message_type="error")
    except (FileNotFoundError, PermissionError) as e:
        error_msg = f"File system error during first-time setup: {e!s}"
        print_message(error_msg, message_type="error")
    except ValueError as e:
        error_msg = f"Invalid argument passed to first-time setup script: {e!s}"
        print_message(error_msg, message_type="error")
...
if check_google_colab():
    try:
        with open("/content/alltalk_tts/googlecolab.json", encoding="utf-8") as f:
            data = json.load(f)
...
if _state["running_on_google_colab"]:
    ...
    print_message("\033[94mGradio Dark :\033[00m \033[92m" + _state["tunnel_url_2"] + "?__theme=dark\033[00m")
    print_message("")
else:
    print_message("")
    ...
    if config.launch_gradio:
        print_message(
            "\033[94mGradio Light:\033[00m \033[92mhttp://127.0.0.1:" + str(config.gradio_port_number) + "\033[00m"
        )
        print_message(
...
def check_espeak_ng():
    ...
    try:
        subprocess.run(["espeak-ng", "--version"], capture_output=True, text=True, check=True)
        return
    except FileNotFoundError:
        if platform.system() == "Windows":
            print_espeak_warning("Windows", r"this location \033[93m\alltalk_tts\system\espeak-ng\\")
            print_message(
            ...
            print_espeak_warning("macOS", "\033[93mHomebrew: brew install espeak-ng\033[0m")
        else:  # Linux
...
def get_alltalk_settings():
    ...
    try:
        ...
        with requests.Session() as session:
            responses = {name: session.get(url, timeout=connection_timeout) for name, url in endpoints.items()}

        # Check if all responses are successful
        if not all(r.status_code == 200 for r in responses.values()):
            failed_endpoints = [name for name, r in responses.items() if r.status_code != 200]
            print_message(
        ...
        return {
            **base_settings,
            **{k: settings_data.get(k, v) for k, v in default_settings.items() if k not in base_settings},
        }

    except (RequestException, RequestsConnectionError) as e:
        print_message(
            f"Unable to connect to the {config.branding} API server: {e!s}",
            message_type="warning",
            component="TTS",
        )
        print_message(
    ...
    if os.path.exists(uploads_dir):
        for file in os.listdir(uploads_dir):
            ...
            try:
                ...
                error_msg = f"OS error while deleting {file}: {e!s}"
                print_message(error_msg, "error")
...
def process_audio_files(
    ...
    try:
        ...
        for audio_file in audio_files:
            ...
            validation_messages.extend([f"Warning for {file_name}: {warning}" for warning in transcribe_warnings])
        ...
        for idx, audio_file in enumerate(tqdm(processed_files, desc="Processing files")):
            try:
                ...
                error_msg = f"Error in transcription of {Path(audio_file).name}: {e!s}"
                print_message(error_msg, "error")
                validation_messages.append(error_msg)
                continue
            except OSError as e:
                error_msg = f"File error processing {Path(audio_file).name}: {e!s}"
                print_message(error_msg, "error")
                validation_messages.append(error_msg)
                continue
            except (ValueError, TypeError) as e:
                error_msg = f"Data processing error in {Path(audio_file).name}: {e!s}"
                print_message(error_msg, "error")
                validation_messages.append(error_msg)
                continue
        ...
        status_message = "\n".join(
            validation_messages + [f"Processed {m['original_file']}: {m['duration']:.2f} seconds" for m in metadata]
        )
        ...
        print_message(f"File system error: {e!s}", "error")
        return None, f"File system error: {e!s}"
    except torch.cuda.OutOfMemoryError as e:
        print_message(f"GPU memory error: {e!s}", "error")
        return None, "GPU memory error: Consider using a smaller model"
    except ValueError as e:
        print_message(f"Invalid input error: {e!s}", "error")
        return None, f"Configuration error: {e!s}"
    finally:
        # Cleanup GPU memory
...
def reset_audio_stream_handlers(dictate_audio, state, text_output, audio_plot):
    ...
    dictate_audio.start_recording(
        ...
        inputs=[state],
        outputs=[state],  # pylint: disable=undefined-variable
...
def visualize_audio_levels(
    ...
    fig.add_trace(go.Scatter(x=time_points, y=rms_values, mode="lines", name="Audio Level"))
...
def process_audio(audio, state):
    ...
    try:
        ...
        error_msg = f"Error processing audio data: {e!s}"
    except np.VisualWarning as e:
        error_msg = f"Error in numerical operations: {e!s}"
    except RuntimeError as e:
        error_msg = f"Error in speech recognition: {e!s}"
    except OSError as e:
        error_msg = f"Error saving transcript: {e!s}"
    print_message(error_msg, "error")
    return default_return
...
if gradio_enabled is True:
    ...
    def dynamic_import(module_path, package=None):
        ...
        try:
            ...
            print_message(f"Module not found: {module_path} - {e!s}", message_type="error")
            return None
        except (ImportError, SyntaxError) as e:
            print_message(f"Error importing {module_path}: {e!s}", message_type="error")
            return None
    ...
    def at_update_dropdowns():
        ...
        language_label = (
            "Languages" if _state["srv_settings_capabilities"]["languages_capable"] else "Model not multi language"
        )
    ...
    def change_model_loaded(new_engine_name, new_selected_model):
        ...
        try:
            ...
            with open(tts_engines_file, encoding="utf-8") as change_model_f:
    ...
    def generate_tts(
        ...
        if gen_text == "":
            ...
            return None, "No Text was sent to generate as TTS"
        ...
        if gen_stream == "true":
            ...
            # pylint: disable=line-too-long
            streaming_url = (
                f"{api_url}?text={encoded_text}&voice={gen_char}&language={gen_lang}&output_file={gen_filen}"
            )
            ...
            try:
                ...
                if retries == max_retries:
                    if isinstance(e, requests.exceptions.Timeout):
                        ...
                        print_message(f"Error Details: {e!s}", message_type="error")
    ...
    def alltalk_gradio():  # pylint: disable=too-many-statements, too-many-branches
        ...
        with gr.Blocks(
                    ...
                    with gr.Row():
                        ...
                        gen_narren = gr.Dropdown(
                            ...
                            value=(
                                ...
                                else ("silent" if config.api_def.api_narrator_enabled == "silent" else "false")
                            ),
                            allow_custom_value=True,
                        )
            ...
            with gr.Tab("Transcribe"):
                ...
                delete_btn.click(fn=delete_uploaded_files, inputs=[], outputs=[status_output])
            ...
            with gr.Tab("Dictate"):
                ...
                with gr.Accordion("Advanced Settings (Change Before Loading The Model)", open=False):
                    with gr.Row():
                        ...
                        add_timestamps = gr.Dropdown(label="Add Timestamps", choices={False, True}, value=False)
                    ...
                    with gr.Row():
                        enable_audio_processing = gr.Checkbox(label="Enable ALL Audio Enhancements", value=True)
                        bandpass_filter = gr.Checkbox(label="Apply Bandpass Filter", value=True)
                        noise_reduction = gr.Checkbox(label="Apply Noise Reduction", value=True)
                        compression = gr.Checkbox(label="Apply Audio Compression", value=True)
                ...
                dictate_audio.start_recording(fn=on_start_recording, inputs=[state], outputs=[state]).success(
                    fn=None, js="() => {console.log('Recording started');}"
                )

                dictate_audio.stop_recording(fn=on_stop_recording, inputs=[state], outputs=[state]).success(
                    fn=None, js="() => {console.log('Recording stopped');}"
                )
            ...
            with gr.Tab("Global Settings"):
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            ...
                            gradio_interface = gr.Dropdown(
                                ...
                                value=("Enabled" if config.gradio_interface else "Disabled"),
                                    ...
                                    api_narrator_enabled = gr.Dropdown(
                                        ...
                                        value=(
                                            ...
                                            if config.api_def.api_narrator_enabled == "true"
                                                ...
                                                "silent" if config.api_def.api_narrator_enabled == "silent" else "false"
                                            )
                                        ),
                                    )
                            ...
                            with gr.Row():
                                ...
                                if rvc_char_model_file_default not in rvc_model_file_available:
                                    rvc_char_model_file_default = (
                                        rvc_model_file_available[0] if rvc_model_file_available else ""
                                    )
                ...
                with gr.Tab("SSL Proxy Server"):
                    ...
                    with gr.Accordion("HELP - 🎯 Proxy Quick Start Guide", open=False):
                        gr.Markdown(AllTalkHelpContent.PROXY, elem_classes="custom-markdown")
                        with gr.Row():
                            gr.Markdown(
                ...
                with gr.Tab("Text-generation-webui Settings"):
                    with gr.Row():
                        activate = gr.Dropdown(
                            choices={"Enabled": "true", "Disabled": "false"},
                            label="Activate TTS",
                            value=("Enabled" if config.tgwui.tgwui_activate_tts else "Disabled"),
                            allow_custom_value=True,
                        )
                        autoplay = gr.Dropdown(
                            choices={"Enabled": "true", "Disabled": "false"},
                            label="Autoplay TTS",
                            value=("Enabled" if config.tgwui.tgwui_autoplay_tts else "Disabled"),
                            allow_custom_value=True,
                        )
                        show_text = gr.Dropdown(
                            choices={"Enabled": "true", "Disabled": "false"},
                            label="Show Text",
                            value=("Enabled" if config.tgwui.tgwui_show_text else "Disabled"),
                            allow_custom_value=True,
                        )
                        narrator_enabled = gr.Dropdown(
                            ...
                            value=(
                                ...
                                else ("silent" if config.tgwui.tgwui_narrator_enabled == "silent" else "false")
                            ),
                            allow_custom_value=True,
                        )
                        ...
                        for engine_name in _state.get("srv_engines_available", []):
                            ...
                            if module:
                                with gr.Tab(f"{engine_name.capitalize()} TTS"):
                                    ...
                                    getattr(module, f"{engine_name}_at_gradio_settings_page")(
