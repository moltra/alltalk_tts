...
def generate_tts(port, engine_num, text, voice):
    try:
        response = requests.post(
            f"http://127.0.0.1:{port}/api/tts-generate",
            data={
                "text_input": text,
            ...
            timeout=30,
        )  # Increased timeout for TTS generation
        data = response.json()
        if data["status"] == "generate-success":
            return (
                f"http://127.0.0.1:{port}{data['output_file_url']}",
                f"TTS generated successfully on Engine {engine_num} and Port {port}",
            )
        ...
        return None, f"Unexpected error while generating TTS on port {port}: {e!s}"
...
def test_tts(engine, voice, text):
    ...
    try:
        ...
        return None, f"Error in test_tts: {e!s}"
    ...
    with gr.Blocks(title="AllTalk Multi Engine Manager", theme=gr.themes.Base()) as interface:
        ...
        with gr.Tab("Engine Management"):
            with gr.Row():
                num_instances_input = gr.Slider(
                    minimum=1, maximum=config.max_instances, step=1, value=1, label="Number of Instances to Start"
                )
                    ...
                    for j in range(8):
                        if i + j < config.max_instances:
                            ...
                            with gr.Column(scale=1, min_width=160):
                                ...
                                start_btn.click(
                                    lambda id=instance_id: start_and_update(id, config.base_port + id - 1),
                                    outputs=status,
                                )
                                stop_btn.click(lambda id=instance_id: stop_and_update(id), outputs=status)
                                restart_btn.click(
                                    lambda id=instance_id: restart_and_update(id, config.base_port + id - 1),
                                    outputs=status,
                                )
            ...
            refresh_button.click(refresh_and_update_slider, outputs=[num_instances_input] + instance_statuses)

            def validate_and_start(num_instances):
                if num_instances < 1:
                    return (
                        [gr.update(value=1)]
                        + ["❌ Not running"] * config.max_instances
        ...
        with gr.Tab("Engines TTS Test"):
            with gr.Row():
                engine_selector = gr.Dropdown(
                    choices=["Please Start an Engine & Refresh List"],
                    value="Please Start an Engine & Refresh List",
                ...
                test_text = gr.Textbox(
                    ...
                    value="This is a test of TTS engine number {engine} on port {port}, using the {voice} voice.",
                    scale=2,
                )
            ...
            def update_engine_selector():
                ...
                debug_str = "\n".join(debug_info)
                return gr.update(
                    choices=running_engines, value=running_engines[0] if running_engines else None
                ), debug_str
            ...
            engine_selector.change(
                update_voice_selector, inputs=[engine_selector], outputs=[voice_selector, debug_output]
            )

        # Generate TTS when test button is clicked
        test_button.click(
            test_tts, inputs=[engine_selector, voice_selector, test_text], outputs=[audio_output, debug_output]
        )
        ...
        with gr.Tab("MEM Settings"):
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        base_port_input = gr.Number(
                            value=config.base_port, label=f"Starting Port Number (Current: {config.base_port})", step=1
                        )
                        ...
                        auto_start_engines = gr.Number(
                            value=config.auto_start_engines, label="Auto-start Engine Instances (0 to disable)", step=1
                        )
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        max_retries_input = gr.Number(
                            value=config.max_retries, label=f"Max Retries (Current: {config.max_retries})", step=1
                        )
                        initial_wait_input = gr.Number(
                            value=config.initial_wait, label=f"Initial Wait (Current: {config.initial_wait})", step=0.1
                        )
            ...
            def update_settings(
                ...
                new_debug_mode,
                new_max_queue_time,
                new_queue_check_interval,
                new_tts_request_timeout,
                new_text_length_factor,
                new_concurrent_request_factor,
                new_diminishing_factor,
                new_queue_position_factor,
            ):
                if new_port < 1024:
                    new_port = 7001
                ...
                return (
                    ...
                    gr.update(
                        value=config.api_server_port, label=f"API Server Port (Current: {config.api_server_port})"
                    ),
            ...
            set_settings_button.click(
                ...
                inputs=[
                    ...
                    auto_start_engines,
                    gradio_port_input,
                    api_server_port_input,
                    max_retries_input,
                    initial_wait_input,
                    backoff_factor_input,
                    debug_mode_input,
                    max_queue_time_input,
                    queue_check_interval_input,
                    tts_request_timeout_input,
                    text_length_factor_input,
                    concurrent_request_factor_input,
                    diminishing_factor_input,
                    queue_position_factor_input,
                ],
                outputs=[
                    settings_status,
                    base_port_input,
        ...
        with gr.Tab("MEM Queue Monitor"):
                    ...
                    for instance, info in tts_instances.items():
                        ...
                        if instance in processes and processes[instance].poll() is None:
                            if info["locked"] and engine_status.current_request:
                                ...
                                text = (
                                    engine_status.current_request[:50] + "..."
                                    if len(engine_status.current_request) > 50
                                    else engine_status.current_request
                                )
                            else:
                                status = "Idle"
                                proc_time = ""
                        ...
                        engine_data.append([f"Engine {instance}", info["port"], status, proc_time, char_count, text])
            ...
            start_button.click(start_monitoring, outputs=[is_monitoring, start_button, stop_button]).then(
                update_monitor_data,
                inputs=[is_monitoring],
                outputs=[queue_length, running_engines, queue_status, engine_status],
            )

            stop_button.click(stop_monitoring, outputs=[is_monitoring, start_button, stop_button]).then(
                lambda: ("0", "0", [], []), outputs=[queue_length, running_engines, queue_status, engine_status]
            )
        ...
        with gr.Tab("MEM Load Test"):
            ...
            with gr.Row():
                ...
                text_length = gr.Slider(
                    minimum=10, maximum=1000, value=100, step=10, label="Approximate Text Length (chars)", scale=2
                )
            ...
            refresh_voices_button.click(refresh_voices, outputs=[voice_dropdown])
...
class ServerThread(threading.Thread):
    def __init__(self, app):
        ...
        self.server = make_server(
            "0.0.0.0", config.api_server_port, app, threaded=True, request_handler=SilentWSGIRequestHandler
        )
